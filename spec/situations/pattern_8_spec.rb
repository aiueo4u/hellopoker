...
describe 'pattern_8' do
  context 'ハンドオープン時にマック' do
    ...
    before do
      ...
      command_runner.run(bb_table_player.player_id, 'PLAYER_ACTION_CHECK')
      command_runner.run(btn_table_player.player_id, 'PLAYER_ACTION_CHECK')
      ...
      # hand_open
      command_runner.run(sb_table_player.player_id, 'PLAYER_ACTION_MUCK_HAND')
      command_runner.run(bb_table_player.player_id, 'PLAYER_ACTION_MUCK_HAND')
    end

    it do
      expect(game_hand.current_state).to eq 'finished'
      expect(btn_table_player.reload.stack).to eq 10200
    end
  end
end
