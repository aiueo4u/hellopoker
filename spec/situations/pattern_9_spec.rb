...
describe 'pattern_9' do
  context 'チェック可能状態でフォールドされた場合' do
    ...
    context 'BBがオプションでフォールドした' do
      ...
      it do
        ...
        expect(sb_table_player.reload.stack).to eq 10100
      end
    end

    context 'フロップでみんなフォールドした' do
      before do
        # preflop
        command_runner.run(utg_table_player.player_id, 'PLAYER_ACTION_FOLD')
        command_runner.run(hj_table_player.player_id, 'PLAYER_ACTION_FOLD')
