...
describe :pattern_2 do
  context 'ショートオールイン' do
    let(:table_player1) { create(:table_player, seat_no: 1, stack: 120) }
    let(:table_player2) { create(:table_player, seat_no: 2, stack: 30) } # sb: forced AI
    let(:table_player3) { create(:table_player, seat_no: 3, stack: 10000) } # bb
    let(:table_player4) { create(:table_player, seat_no: 4, stack: 80) } # utg: AI
    let(:table_player5) { create(:table_player, seat_no: 5, stack: 150) } # hj: call 100
    let(:table_player6) { create(:table_player, seat_no: 6, stack: 150) }
    let(:table_players) {
      [
        table_player1,
        table_player2,
    ...
    before do
      ...
      command_runner.run(co_table_player.player_id, 'PLAYER_ACTION_BET_CHIPS', 150)
      command_runner.run(btn_table_player.player_id, 'PLAYER_ACTION_CALL')
      command_runner.run(bb_table_player.player_id, 'PLAYER_ACTION_CALL')
      command_runner.run(hj_table_player.player_id, 'PLAYER_ACTION_CALL')
    end

    it do
      expect(game_hand.current_state).to eq 'finished'
      expect(btn_table_player.reload.stack).to eq 0
      expect(sb_table_player.reload.stack).to eq 0
      expect(bb_table_player.reload.stack).to eq 9850
      expect(utg_table_player.reload.stack).to eq 0
      expect(hj_table_player.reload.stack).to eq 0
      expect(co_table_player.reload.stack).to eq 680
    end
  end
end
